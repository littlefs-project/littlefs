...
#define LFSR_RATTR_(_tag, _weight, _datas, _data_count) \
    ((lfsr_rattr_t){ \
        .tag=_tag, \
...
#define LFSR_RATTR_RATTRS(_rattrs, _rattr_count) \
    ((lfsr_rattr_t){ \
        .tag=LFSR_TAG_RATTRS, \
        .data_count=(uint16_t){_rattr_count}, \
        .weight=0, \
...
#define LFSR_RATTR_ATTRS(_attrs, _attr_count) \
    ((lfsr_rattr_t){ \
        .tag=LFSR_TAG_ATTRS, \
        .data_count=(uint16_t){_attr_count}, \
        .weight=0, \
...
static int lfsr_mdir_commit(lfs_t *lfs, lfsr_mdir_t *mdir,
    ...
    if (err == LFS_ERR_RANGE) {
        ...
        for (int i = 0; i < 2; i++) {
            ...
            bool l = lfsr_mid_rid(lfs, mdir->mid) < split_rid;
