...
static lfs_ssize_t lfsr_bd_ckprefix(lfs_t *lfs,
    ...
    return hint_ - (off-lfsr_ck_ckoff(ck));
}
...
static lfs_scmp_t lfsr_bd_cmpck_(lfs_t *lfs,
    ...
    while (size_ > 0) {
        const uint8_t *buffer__;
        lfs_size_t size__;
        int err = lfsr_bd_readnext(lfs, block, off_, hint__, size_,
                &buffer__, &size__);
        if (err) {
            return err;
        }
    ...
    // checksum any suffixed data and validate
    int err = lfsr_bd_cksuffix(lfs, block, off+size, hint_-size, ck,
            cksum);
    if (err) {
        return err;
    }
...
static int lfsr_bd_cpyck_(lfs_t *lfs,
    ...
    lfs_ssize_t hint_ = lfsr_bd_ckprefix(lfs, src_block, src_off, hint, ck,
            &src_cksum);
    if (hint_ < 0) {
        return hint_;
    }
    ...
    // checksum any suffixed data and validate
    int err = lfsr_bd_cksuffix(lfs, src_block, src_off+size, hint_-size, ck,
            src_cksum);
    if (err) {
        return err;
    }
...
static lfsr_data_t lfsr_data_frombptr(const lfsr_bptr_t *bptr,
    ...
    // cksize should not exceed 28-bits
    LFS_ASSERT(bptr->data.u.disk.ck.cksize <= 0x0fffffff);
    lfs_ssize_t d = 0;
    ...
    d_ = lfs_toleb128(bptr->data.u.disk.ck.cksize, &buffer[d], 4);
    if (d_ < 0) {
        LFS_UNREACHABLE();
    }
    d += d_;

    lfs_tole32_(bptr->data.u.disk.ck.u.cksum, &buffer[d]);
    d += 4;
...
static int lfsr_data_readbptr(lfs_t *lfs, lfsr_data_t *data,
    ...
    err = lfsr_data_readlleb128(lfs, data, &bptr->data.u.disk.ck.cksize);
    if (err) {
        return err;
    }
