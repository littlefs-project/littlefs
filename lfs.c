...
#define LFS_CFG_READ(lfs, block, off, buffer, size) \
    ((void)lfs, lfs_read(block, off, buffer, size))
#define LFS_CFG_PROG(lfs, block, off, buffer, size) \
    ((void)lfs, lfs_prog(block, off, buffer, size))
#define LFS_CFG_ERASE(lfs, block) \
    ((void)lfs, lfs_erase(block))
...
static int lfs_bd_read(lfs_t *lfs,
    ...
    while (size > 0) {
        ...
        if (size >= hint && off % LFS_CFG_READ_SIZE(lfs) == 0 &&
            ...
            int err = LFS_CFG_READ(lfs, block, off, data, diff);
            if (err) {
                return err;
            }
        ...
        int err = LFS_CFG_READ(lfs, rcache->block,
...
static int lfs_bd_flush(lfs_t *lfs,
    ...
    if (pcache->block != LFS_BLOCK_NULL && pcache->block != LFS_BLOCK_INLINE) {
        ...
        int err = LFS_CFG_PROG(lfs, pcache->block,
...
static int lfs_bd_erase(lfs_t *lfs, lfs_block_t block) {
    LFS_ASSERT(block < LFS_CFG_BLOCK_COUNT(lfs));
    int err = LFS_CFG_ERASE(lfs, block);
    LFS_ASSERT(err <= 0);
    return err;
}
...
static int lfs_dir_getinfo(lfs_t *lfs, lfs_mdir_t *dir,
    ...
    lfs_stag_t tag = lfs_dir_get(lfs, dir, LFS_MKTAG(0x780, 0x3ff, 0),
            LFS_MKTAG(LFS_TYPE_NAME, id, LFS_CFG_NAME_MAX(lfs)+1), info->name);
    if (tag < 0) {
        return (int)tag;
    }
...
static int lfs_file_opencommon(lfs_t *lfs, lfs_file_t *file,
    ...
    for (unsigned i = 0; i < LFS_FILE_CFG_ATTR_COUNT(file); i++) {
        ...
        if ((file->flags & 3) != LFS_O_RDONLY) {
            if (LFS_FILE_CFG_ATTRS(file)[i].size > LFS_CFG_ATTR_MAX(lfs)) {
                err = LFS_ERR_NOSPC;
                goto cleanup;
            }
...
int lfs_file_opencfg(lfs_t *lfs, lfs_file_t *file,
    ...
    int err = lfs_file_opencommon(lfs, file, path, flags, cfg);
...
lfs_ssize_t lfs_file_write(lfs_t *lfs, lfs_file_t *file,
    ...
    if (file->pos + size > LFS_CFG_FILE_MAX(lfs)) {
...
lfs_ssize_t lfs_getattr(lfs_t *lfs, const char *path,
    ...
    tag = lfs_dir_get(lfs, &cwd, LFS_MKTAG(0x7ff, 0x3ff, 0),
            LFS_MKTAG(LFS_TYPE_USERATTR + type,
                id, lfs_min(size, LFS_CFG_ATTR_MAX(lfs))),
            buffer);
    if (tag < 0) {
        if (tag == LFS_ERR_NOENT) {
...
static int lfs_initcommon(lfs_t *lfs
...
static int lfs_formatcommon(lfs_t *lfs
#ifndef LFS_STATICCFG
        , const struct lfs_cfg *cfg
#endif
        ) {
    ...
    {
#ifdef LFS_STATICCFG
        err = lfs_initcommon(lfs);
#else
        err = lfs_initcommon(lfs, cfg);
#endif
        if (err) {
            return err;
        }
        ...
        lfs_superblock_t superblock = {
            ...
            .name_max    = LFS_CFG_NAME_MAX(lfs),
            .file_max    = LFS_CFG_FILE_MAX(lfs),
            .attr_max    = LFS_CFG_ATTR_MAX(lfs),
        };
...
int lfs_format(lfs_t *lfs) {
    LFS_TRACE("lfs_format(%p)", (void*)lfs);
...
int lfs_formatcfg(lfs_t *lfs, const struct lfs_cfg *cfg) {
    ...
    int err = lfs_formatcommon(lfs, cfg);
    LFS_TRACE("lfs_formatcfg -> %d", err);
    return err;
}
#endif

static int lfs_mountcommon(lfs_t *lfs
#ifndef LFS_STATICCFG
        , const struct lfs_cfg *cfg
#endif
        ) {
#ifdef LFS_STATICCFG
    int err = lfs_initcommon(lfs);
#else
    int err = lfs_initcommon(lfs, cfg);
#endif
    if (err) {
        return err;
    }
    ...
    while (!lfs_pair_isnull(dir.tail)) {
        ...
        if (tag && !lfs_tag_isdelete(tag)) {
            ...
            if (superblock.name_max) {
                if (superblock.name_max > LFS_CFG_NAME_MAX(lfs)) {
                    LFS_ERROR("Unsupported name_max (%"PRIu32" > %"PRIu32")",
                            superblock.name_max, LFS_CFG_NAME_MAX(lfs));
                    err = LFS_ERR_INVAL;
                    goto cleanup;
                }

                lfs->cfg.name_max = superblock.name_max;
            }

            if (superblock.file_max) {
                if (superblock.file_max > LFS_CFG_FILE_MAX(lfs)) {
                    LFS_ERROR("Unsupported file_max (%"PRIu32" > %"PRIu32")",
                            superblock.file_max, LFS_CFG_FILE_MAX(lfs));
                    err = LFS_ERR_INVAL;
                    goto cleanup;
                }

                lfs->cfg.file_max = superblock.file_max;
            }

            if (superblock.attr_max) {
                if (superblock.attr_max > LFS_CFG_ATTR_MAX(lfs)) {
                    LFS_ERROR("Unsupported attr_max (%"PRIu32" > %"PRIu32")",
                            superblock.attr_max, LFS_CFG_ATTR_MAX(lfs));
                    err = LFS_ERR_INVAL;
                    goto cleanup;
                }

                lfs->cfg.attr_max = superblock.attr_max;
            }
        }
...
int lfs_mount(lfs_t *lfs) {
    LFS_TRACE("lfs_mount(%p)", (void*)lfs);
...
int lfs_mountcfg(lfs_t *lfs, const struct lfs_cfg *cfg) {
    LFS_TRACE("lfs_mountcfg(%p, %p {.ctx=%p, "
            ...
            cfg->read_buffer, cfg->prog_buffer, cfg->lookahead_buffer,
            cfg->name_max, cfg->file_max, cfg->attr_max);
    int err = lfs_mountcommon(lfs, cfg);
    LFS_TRACE("lfs_mountcfg -> %d", err);
    return err;
}
#endif
...
/// Filesystem operations ///
static int lfs1_mountcommon(lfs_t *lfs, struct lfs1 *lfs1) {
    int err = 0;
    {
        err = lfs_initcommont(lfs);
        if (err) {
            return err;
        }
...
/// v1 migration ///
static int lfs_migratecommon(lfs_t *lfs) {
    struct lfs1 lfs1;
    int err = lfs1_mountcommon(lfs, &lfs1);
    if (err) {
        return err;
    }
...
#if defined(LFS_STATICCFG)
int lfs_migrate(lfs_t *lfs) {
    LFS_TRACE("lfs_migrate(%p)", (void*)lfs);
