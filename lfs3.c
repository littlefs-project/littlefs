...
#define LFS3_RATTR_5(_len, _tag, _weight, _from, _count) \
    (((lfs3_rattr_t)(_len) << 26) \
        | ((lfs3_rattr_t)(_tag) << 0) \
        | ((lfs3_rattr_t)(_weight) << 30) \
        | ((lfs3_rattr_t)(_from) << 22) \
        | ((lfs3_rattr_t)(_count) << 16))

#define LFS3_RATTR_4(_len, _tag, _weight, _from) \
    LFS3_RATTR_5(_len, _tag, _weight, _from, 0)

#define LFS3_RATTR_3(_len, _tag, _weight) \
    LFS3_RATTR_5(_len, _tag, _weight, LFS3_FROM_NIL, 0)
...
static int lfs3_rbyd_appendrattr_(lfs3_t *lfs3, lfs3_rbyd_t *rbyd,
    ...
    // nil data?
    if (from == LFS3_FROM_NIL) {
        datas = NULL;
        data_count = 0;

    // small buffer?
    } else if (from == LFS3_FROM_BUF) {
        ctx.u.data = LFS3_DATA_BUF((const uint8_t*)args[0], count);
        datas = &ctx.u.data;
        data_count = 1;
    ...
    } else if (from == LFS3_FROM_CAT) {
        datas = (const lfs3_data_t*)args[0];
        data_count = count;

    // le32?
    } else if (from == LFS3_FROM_LE32) {
...
static int lfs3_rbyd_appendcompactrbyd(lfs3_t *lfs3, lfs3_rbyd_t *rbyd_,
    ...
    while (true) {
        ...
        // write the tag
        int err = lfs3_rbyd_appendcompactrattr(lfs3, rbyd_, LFS3_RATTRS(
                LFS3_RATTR(3, tag, -2, LFS3_FROM_CAT, 1),
...
static int lfs3_bshrub_fetch_(lfs3_t *lfs3, const lfs3_mdir_t *mdir,
        lfs3_btree_t *btree) {
...
static int lfs3_bshrub_commitroot_(lfs3_t *lfs3, lfs3_bshrub_t *bshrub,
    ...
    for (const lfs3_rattr_t *r = bcommit->rattrs;
            *r;
...
static inline bool lfs3_t_islookgbmap(uint32_t flags) {
    (void)flags;
    #if !defined(LFS3_RDONLY) && defined(LFS3_GBMAP)
    return flags & LFS3_T_LOOKGBMAP;
    #else
    return false;
    #endif
}
...
static int lfs3_rbyd_appendgdelta(lfs3_t *lfs3, lfs3_rbyd_t *rbyd) {
    ...
    if (lfs3_memlen(grmdelta_, LFS3_GRM_DSIZE) != 0) {
        ...
        int err = lfs3_rbyd_appendrattr(lfs3, rbyd, -1, LFS3_RATTRS(
                // opportunistically remove this tag if delta is all zero
                (size == 0)
                    ? LFS3_RATTR(2, LFS3_tag_RM | LFS3_TAG_GRMDELTA, 0)
                    : LFS3_RATTR(2, LFS3_TAG_GRMDELTA, 0, LFS3_FROM_BUF, size),
    ...
    if (lfs3_f_isgbmap(lfs3->flags)) {
        ...
        if (lfs3_memlen(gbmapdelta_, LFS3_GBMAP_DSIZE) != 0) {
            ...
            int err = lfs3_rbyd_appendrattr(lfs3, rbyd, -1, LFS3_RATTRS(
                    // opportunistically remove this tag if delta is all zero
                    (size == 0)
                        ? LFS3_RATTR(2, LFS3_tag_RM | LFS3_TAG_GBMAPDELTA, 0)
                        : LFS3_RATTR(2, LFS3_TAG_GBMAPDELTA, 0,
                            LFS3_FROM_BUF, size),
    ...
    if (rid >= start_rid
        ...
        for (; *r; r = lfs3_rattr_next(r, &rid)) {
            ...
            // in our rbyd
                ...
                lfs3_bshrub_t *bshrub_ = (lfs3_bshrub_t*)lfs3_rattr_arg(r, 0);
                lfs3_srid_t rid_ = lfs3_rattr_arg(r, 1);
                const lfs3_rattr_t *rattrs_
                ...
                // commit to shrub
                int err = lfs3_shrub_commit(lfs3, &mdir_->r, &bshrub_->b_,
                        rid_, rattrs_);
                if (err) {
                    return err;
                }
                ...
                while (true) {
                    ...
                    if (tag == LFS3_TAG_BSHRUB) {
                        ...
                        err = lfs3_rbyd_appendrattr(lfs3, &mdir_->r,
                                rid - lfs3_smax(start_rid, 0), LFS3_RATTRS(
                                    LFS3_RATTR(2, LFS3_TAG_BSHRUB, 0,
                                        LFS3_FROM_SHRUB),
                    ...
                    // append the rattr
                        ...
                        int err = lfs3_rbyd_appendrattr(lfs3, &mdir_->r,
                                rid - lfs3_smax(start_rid, 0), LFS3_RATTRS(
                                    LFS3_RATTR(2, tag, 0, LFS3_FROM_CAT, 1),
                                    LFS3_RATTR_ARG(&data)));
                        if (err) {
                            return err;
                        }
...
static int lfs3_mdir_commit(lfs3_t *lfs3, lfs3_mdir_t *mdir,
    ...
    // commit to mdir
    return lfs3_mdir_commit_(lfs3, mdir, rattrs);
}
#endif
...
static int lfs3_mdir_compact_(lfs3_t *lfs3, lfs3_mdir_t *mdir) {
    ...
    lfs3_mdir_claim(mdir);
    return lfs3_mdir_commit_(lfs3, mdir, LFS3_RATTRS(LFS3_RATTR_NULL));
}
#endif
...
again:;
    ...
    if (mtrv->u.btrv.bid == LFS3_BID_MDIR) {
        ...
        // fetch the next btree/bshrub
            ...
            // that's ok
            int err = lfs3_bshrub_fetch_(lfs3, &mtrv->h.mdir, &mtrv->b);
            if (err && err != LFS3_ERR_NOENT) {
                return err;
            }
        }
...
static lfs3_stag_t lfs3_mtree_gc(lfs3_t *lfs3, lfs3_mgc_t *mgc,
    ...
    if (mgc->t.h.mdir.mid == LFS3_MID_MROOTANCHOR) {
        ...
        //
        // note we try to repopupate even if the lookahead flag isn't
        ...
        //
        // note we _don't_ try to repopulate if the lookgbmap flag isn't
        // set because repopulating the gbmap requires disk writes and
        ...
        if (lfs3_t_islookgbmap(mgc->t.h.flags)
                && lfs3_f_isgbmap(lfs3->flags)
                && lfs3_t_islookgbmap(lfs3->flags)
                && !lfs3_t_ismtreeonly(mgc->t.h.flags)
                && !lfs3_t_isckpointed(mgc->t.h.flags)) {
...
again:;
    ...
    if (lfs3_t_ismkconsistent(mgc->t.h.flags)
        ...
        if (mdir->mid >= 0 && mdir->r.weight == 0) {
            // continue traversal
            // TODO big hack! is it big enough?
            mgc->t.h.mdir.mid -= 1;
...
eot:;
    ...
    if (LFS3_IFDEF_GBMAP(
            lfs3_t_islookgbmap(mgc->t.h.flags)
                && lfs3_f_isgbmap(lfs3->flags)
                && lfs3_t_islookgbmap(lfs3->flags)
                && !lfs3_t_ismtreeonly(mgc->t.h.flags)
                && !lfs3_t_isckpointed(mgc->t.h.flags),
    ...
    } else if (lfs3_t_islookahead(mgc->t.h.flags)
            && !lfs3_t_ismtreeonly(mgc->t.h.flags)
            && !lfs3_t_isckpointed(mgc->t.h.flags)) {
...
#if !defined(LFS3_RDONLY) && defined(LFS3_GBMAP)
static inline bool lfs3_alloc_islookgbmap(const lfs3_t *lfs3) {
...
static void lfs3_alloc_inc(lfs3_t *lfs3) {
    ...
    if (lfs3_f_isgbmap(lfs3->flags)) {
        ...
        if (lfs3_alloc_islookgbmap(lfs3)) {
...
int lfs3_mkdir(lfs3_t *lfs3, const char *path) {
    ...
    err = lfs3_mdir_commit(lfs3, &mdir, LFS3_RATTRS(
            ...
            LFS3_RATTR_ARG(did_),
            LFS3_RATTR(1, LFS3_tag_GRMPUSH, 0),
            LFS3_RATTR_NULL));
    if (err) {
        return err;
    }
...
int lfs3_setattr(lfs3_t *lfs3, const char *path, uint8_t type,
    ...
    // commit our attr
    err = lfs3_mdir_commit(lfs3, &mdir, LFS3_RATTRS(
            LFS3_RATTR(3, LFS3_TAG_ATTR(type), 0, LFS3_FROM_DATA),
            LFS3_RATTR_ARG(size),
...
int lfs3_removeattr(lfs3_t *lfs3, const char *path, uint8_t type) {
    ...
    // commit our removal
    err = lfs3_mdir_commit(lfs3, &mdir, LFS3_RATTRS(
            LFS3_RATTR(1, LFS3_tag_RM | LFS3_TAG_ATTR(type), 0),
            LFS3_RATTR_NULL));
    if (err) {
        return err;
    }
...
static int lfs3_file_graft_(lfs3_t *lfs3, lfs3_file_t *file,
    ...
    if (pos > file->b.b.r.weight) {
        ...
        if (file->b.b.r.weight > 0) {
            ...
            *r++ = LFS3_RATTR(2, LFS3_tag_GROW, -2);
            *r++ = LFS3_RATTR_WEIGHT(+(pos - file->b.b.r.weight));

        // new hole
        } else {
            bid = lfs3_min(bid, file->b.b.r.weight);
            *r++ = LFS3_RATTR(2, LFS3_TAG_DATA, -2);
            *r++ = LFS3_RATTR_WEIGHT(+(pos - file->b.b.r.weight));
        }
    }
    ...
    while (pos < file->b.b.r.weight) {
        ...
        if (bid-(weight_-1) < pos) {
            ...
            // carve fragment?
                ...
                *r++ = LFS3_RATTR(3,
                        LFS3_tag_GROW | LFS3_tag_MASK8 | LFS3_TAG_DATA, -2,
                        LFS3_FROM_CAT, 1);
                *r++ = LFS3_RATTR_WEIGHT(-(bid+1 - pos));
            ...
            // carve bptr?
                ...
                *r++ = LFS3_RATTR(3,
                        LFS3_tag_GROW | LFS3_tag_MASK8 | LFS3_TAG_BLOCK, -2,
    ...
    if (weight + delta > 0) {
        ...
        // append a new fragment?
            ...
            bid = lfs3_min(bid, file->b.b.r.weight);
            *r++ = LFS3_RATTR(3, LFS3_TAG_DATA, -2,
        ...
        // append a new bptr?
            ...
            bid = lfs3_min(bid, file->b.b.r.weight);
            *r++ = LFS3_RATTR(3, LFS3_TAG_BLOCK, -2,
                    LFS3_FROM_BPTR, LFS3_BPTR_DSIZE);
            *r++ = LFS3_RATTR_WEIGHT(+(weight + delta));
            *r++ = LFS3_RATTR_ARG(graft);
        }
...
static int lfs3_file_sync_(lfs3_t *lfs3, lfs3_file_t *file,
    ...
    if (lfs3_o_isunsync(file->b.h.flags)) {
        ...
        if (lfs3_o_isunflush(file->b.h.flags)) {
            ...
            if (file->cache.size > 0) {
                ...
                *shrub_r++ = LFS3_RATTR(4, LFS3_TAG_DATA, -2, LFS3_FROM_DATA);
                ...
                *r++ = LFS3_RATTR(4, LFS3_tag_SHRUBCOMMIT, 0);
                *r++ = LFS3_RATTR_ARG(&file->b);
                *r++ = LFS3_RATTR_ARG(0);
    ...
    if (attrs) {
        ...
        *r++ = LFS3_RATTR(3, LFS3_tag_ATTRS, 0);
...
int lfs3_mount(lfs3_t *lfs3, uint32_t flags,
    ...
    LFS3_ASSERT(!lfs3_m_isrdonly(flags) || !lfs3_t_islookgbmap(flags));
    ...
    LFS3_INFO("Mounted littlefs v%"PRId32".%"PRId32" %"PRId32"x%"PRId32" "
            ...
            lfs3->mtree.r.weight >> lfs3->mbits,
            1 << lfs3->mbits,
            lfs3->gcksum);
...
static int lfs3_formatinited(lfs3_t *lfs3) {
    ...
    for (int i = 0; i < 2; i++) {
        ...
        //
        lfs3_rattr_t rattrs[LFS3_IFDEF_GBMAP(19, 17)];
        ...
        const lfs3_geometry_t geometry = {
            ...
            .block_count=lfs3->cfg->block_count,
        };

        // magic + various config
        lfs3_rattr_t *r = rattrs;
        *r++ = LFS3_RATTR(2, LFS3_TAG_MAGIC, 0, LFS3_FROM_BUF, 8);
        *r++ = LFS3_RATTR_ARG("littlefs");
        *r++ = LFS3_RATTR(2, LFS3_TAG_VERSION, 0, LFS3_FROM_BUF, 2);
        *r++ = LFS3_RATTR_ARG(version);
        *r++ = LFS3_RATTR(2, LFS3_TAG_RCOMPAT, 0, LFS3_FROM_LE32);
        *r++ = LFS3_RATTR_ARG(lfs3_rcompat(lfs3));
        *r++ = LFS3_RATTR(2, LFS3_TAG_WCOMPAT, 0, LFS3_FROM_LE32);
        ...
        *r++ = LFS3_RATTR_ARG(lfs3->file_limit);
        ...
        if (lfs3_f_isgbmap(lfs3->flags)) {
            ...
            *r++ = LFS3_RATTR(2, LFS3_TAG_GBMAPDELTA, 0,
                    LFS3_FROM_BUF,
                    lfs3_memlen(lfs3->gbmap_d, LFS3_GBMAP_DSIZE));
            *r++ = LFS3_RATTR_ARG(&lfs3->gbmap_d);
        }
        #endif
        // root did=0
        *r++ = LFS3_RATTR(2, LFS3_TAG_BOOKMARK, +1, LFS3_FROM_LEB128);
        *r++ = LFS3_RATTR_ARG(0);
        *r++ = LFS3_RATTR_NULL;
        LFS3_ASSERT((lfs3_size_t)(r-rattrs)
                <= sizeof(rattrs)/sizeof(lfs3_rattr_t));

        err = lfs3_rbyd_appendrattrs(lfs3, &rbyd, -1, -1, -1, rattrs);
        if (err) {
            return err;
        }
...
int lfs3_fs_stat(lfs3_t *lfs3, struct lfs3_fsinfo *fsinfo) {
    ...
    fsinfo->flags = lfs3->flags & (
            LFS3_I_RDONLY
                ...
                | LFS3_IFDEF_RDONLY(0, LFS3_I_MKCONSISTENT)
                | LFS3_IFDEF_RDONLY(0, LFS3_I_LOOKAHEAD)
                | LFS3_IFDEF_RDONLY(0, LFS3_IFDEF_GBMAP(LFS3_I_LOOKGBMAP, 0))
                | LFS3_IFDEF_RDONLY(0, LFS3_I_COMPACTMETA)
                | LFS3_I_CKMETA
                | LFS3_I_CKDATA
...
static int lfs3_fs_gc_(lfs3_t *lfs3, lfs3_mgc_t *mgc,
    ...
    while (pending && (lfs3_off_t)steps > 0) {
        ...
        if (lfs3_t_isckpointed(mgc->t.h.flags)) {
            ...
            #ifdef LFS3_GBMAP
            mgc->t.h.flags &= ~LFS3_T_LOOKGBMAP;
            #endif
        }
        #endif
...
int lfs3_fs_gc(lfs3_t *lfs3) {
    ...
    LFS3_ASSERT(!lfs3_m_isrdonly(lfs3->flags)
            || !lfs3_t_islookgbmap(lfs3->cfg->gc_flags));
    LFS3_ASSERT(!lfs3_m_isrdonly(lfs3->flags)
...
int lfs3_trv_open(lfs3_t *lfs3, lfs3_trv_t *trv, uint32_t flags) {
    ...
    LFS3_ASSERT((flags & ~(
            LFS3_IFDEF_RDONLY(0, LFS3_T_RDWR)
                ...
                | LFS3_IFDEF_RDONLY(0,
                    LFS3_IFDEF_GBMAP(LFS3_T_LOOKGBMAP, 0))
    ...
    LFS3_ASSERT(!lfs3_t_isrdonly(flags) || !lfs3_t_islookgbmap(flags));
    ...
    LFS3_ASSERT(!lfs3_t_ismtreeonly(flags) || !lfs3_t_islookgbmap(flags));
